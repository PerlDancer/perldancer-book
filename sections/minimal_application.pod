...
Dancer allows us to create a I<layout> with is a special template that is
...
template. With this layout stored in the layouts directory
(C<appdir/views/layouts/>) we can do change our hello.tt template to just the
following:
