...
As you've seen in the hello world example, we declared the route handler with
the C<get> keyword. That keyword is named after the HTTP method it maps. It's
not necessary to have a complete understanding of all the details of the HTTP
